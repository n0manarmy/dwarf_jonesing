...
func _ready():
	...
	signals_manager.connect("location_entered_present_scene", Callable(self, "present_location"))
	signals_manager.connect("player_time_up", Callable(self, "hide_scenes"))
...
#			scene_11_node.show()
			signals_manager.emit_signal("scene_change", self.name, "Scene11", global_data.SCENE_STATE.SHOW)
		scene_12_area2d:
