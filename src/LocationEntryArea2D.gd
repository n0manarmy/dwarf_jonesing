...
func on_area_2d_body_entered(_body: CharacterBody2D):
	...
	var player_pos = travel_path_tile_map.local_to_map(player_sprite.position)
	player_pos.y -= 4
	if debug_this: print(self.name + ".player_pos: ", player_pos)
	signals_manager.emit_signal("location_entered_move_player", self.name, travel_path_tile_map.map_to_local(player_pos))
